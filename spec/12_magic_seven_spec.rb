...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 13 and the previous step is 20' do
      subject(:game) { described_class.new(13) } # Arrange
      previous_step = 20
      it 'returns 7' do
  ...
  describe '#play' do
    context 'when the random number is 0' do
      subject(:game) { described_class.new(0) } # Arrange
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game.play # Act
        expect(result).to eq(7) # Assert
      end
    end

    context 'when the random number is 16' do
      subject(:game) { described_class.new(16) } # Arrange
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game.play # Act
        expect(result).to eq(7) # Assert
    ...
    context 'when the random number is 67' do
      subject(:game) { described_class.new(67) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game.play # Act
        expect(result).to eq(7) # Assert
