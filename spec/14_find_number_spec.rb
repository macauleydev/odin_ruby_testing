...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guessing_5_9) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = game_guessing_5_9.make_guess
        expect(guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guessing_8_9) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = game_guessing_8_9.make_guess
        expect(guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing_0_3) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        guess = game_guessing_0_3.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing_3_3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        guess = game_guessing_3_3.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:answer_found) { double('answer_found', value: 5) }
      subject(:game_found) { described_class.new(0, 9, answer_found, guess = answer_found.value) }
      ...
      it 'is game over' do
        expect(game_found).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      let(:answer_not_found) { double('answer_not_found', value: 4) }
      bad_guess = 8
      subject(:game_not_found) { described_class.new(0, 9, answer_not_found, bad_guess) }

      it 'is not game over' do
        expect(game_not_found).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is more than the answer' do
      subject(:high_guess_game) { described_class.new(0, 3, number_range, 9) }
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:squeeze_up_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { squeeze_up_game.update_range }.to change { squeeze_up_game.min }.to(8)
      end

      it 'does not update max' do
        expect { squeeze_up_game.update_range }.not_to change { squeeze_up_game.max }
      end
    end
    context 'when the guess is 9, min=8, and max=9' do
      subject(:squeeze_down_game) { described_class.new(8, 9, number_range, 9) }

      it 'updates max to the same value as min' do
        expect { squeeze_down_game.update_range }.to change { squeeze_down_game.max }.to(8)
      end

      it 'does not update min' do
        expect { squeeze_down_game.update_range }.not_to change { squeeze_down_game.min }
      end
    end
  end
end
